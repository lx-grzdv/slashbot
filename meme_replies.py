...
MEME_FORCE_FALLBACK_PROMPT = "в чате тихо, команда ушла в глубокий рендер, но макет дожмём"
...
SILENCE_MEME_PROMPT = (
    "в чате тихо уже три часа: все ушли в глубокий рендер; пора выдохнуть, свериться и дожать главное"
)
...
ENCOURAGING_SCHEDULED_MEME = re.compile(
    r"дож(?:м|им|ат)|выв(?:ез|оз)|погнал|поехал|вперёд|соб(?:ер|ир|р)|сдела|готов|движ|двига|дойд|вытян|разбер|"
...
MEME_TEMPLATES = [
    "блэт, {snippet} — зато теперь ясно, что дожимать",
    "POV: в чате {snippet}, дедлайн рычит, а мы уже собрали план",
    "когда {snippet}, выдохнули, сверились и погнали дальше",
    "ахах, {snippet} — фигма пошутила, теперь наша очередь дожать",
    "после «{snippet}» ноукод стал понятнее — движемся",
    "главный крюк сегодня: {snippet}; зато маршрут к «готово» уже виден",
    "да блэт, {snippet} — ничего, S:P9 и не такое вывозил",
    "план сделал крюк: {snippet}; поправили курс и поехали",
    "я: выдохнул\nчат: {snippet}\nмы: собрались и дожали",
    "если {snippet} — значит нашли ещё один шаг к готовому",
    "блэт, {snippet} — перед клиентом зато выйдем уже с готовым планом",
    "честно, {snippet} — не баг, а тренажёр; технику поправили, движемся",
    "ну да, {snippet} — зато теперь мы реально всё контролируем, погнали",
    "S:P9 сегодня: {snippet}. Концепцию нашли, теперь дожмём макет",
    "после «{snippet}» фигма взяла паузу, а мы нет — погнали",
]

MASHUP_TEMPLATES = [
    "сначала {a}, потом {b} — карта дня собрана, погнали",
    "блэт: {a}. и тут же {b}. ничего, план уже крепче",
    "в чате {a}, а параллельно {b} — хаос богатый, зато мы уже знаем куда двигаться",
    "{a}? ок. потом {b} — выдохнули, собрались и дожали",
    "день начался с «{a}», дошёл до «{b}» — маршрут странный, но мы его вывезем",
    "{a}, потом {b}; короче, макет показал нам следующий шаг — дожмём",
]
