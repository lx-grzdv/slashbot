...
def _background_reply_on_cooldown(update: Update) -> bool:
    """True — фоновую реакцию пропускаем (не @ и не reply на бота)."""
    user = update.effective_user
    if not user:
        return False
    key = (update.effective_chat.id, user.id)
    now = time.monotonic()
    last = _last_pasha_background_reply.get(key)
    if last is not None and now - last < PASHA_BACKGROUND_COOLDOWN_SEC:
        print(
            f"⏭️ Cooldown {PASHA_BACKGROUND_COOLDOWN_SEC:.0f}s: "
            f"chat={key[0]} user={key[1]} update={update.update_id}"
        )
        return True
    return False


def _mark_background_reply(update: Update) -> None:
    user = update.effective_user
    if user:
        _last_pasha_background_reply[(update.effective_chat.id, user.id)] = time.monotonic()
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if reply:
        if _background_reply_on_cooldown(update):
            return
